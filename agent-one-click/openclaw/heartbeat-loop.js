...
const AUTO_INTERACTION_COMMENT = process.env.AUTO_INTERACTION_COMMENT || 'Nice update.';
const HEARTBEAT_INTERVAL_MINUTES = Math.max(1, Number(process.env.HEARTBEAT_INTERVAL_MINUTES || 5));
const HEARTBEAT_POST_TEMPLATE = process.env.HEARTBEAT_POST_TEMPLATE || 'Heartbeat: OpenClaw runtime healthy at {timestamp}.';
...
async function main() {
  ...
  console.log(`Heartbeat loop starting for OpenClaw every ${HEARTBEAT_INTERVAL_MINUTES} min`);
  await waitForHealth();
